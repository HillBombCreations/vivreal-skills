...
- **Phase 3 billing rules:** `cancel-confirmed` (in-app cancellations, sent BEFORE access lapses, carries the access-until date + undo; mutually exclusive with `subscription-cancelled-confirm` by construction; episode = `cancellation.requestedAt`, which `reactivateSubscription` nulls, so cancel→reactivate→cancel re-arms); `win-back-30d` (ONE email 30 to 60 days after access ended, free tier only, body matched to the exit-survey reason, the 60-day upper bound doubles as a first-run blast guard; `cancellation.feedback` is deliberately never merged into an email, only the survey KEY reaches the template); `annual-renewal-reminder` (CA ARL advance notice, 30-day lead, excludes already-cancelled plans, never held out, legal obligation; renewal amount wired into the notice). Holdout policy: account-state + legally-required emails opt out; only win-back permits a holdout arm.
...
- **Tenant placement is STORED, never computed.** Tenant placement is STORED, never computed: `resolvePlacement(group)` from `@hillbombcreations/tenant-db/placement` reads `group.dbKey` back and THROWS when it is absent or unroutable. There is no tier mapping and no fallback anywhere in the fleet. A projection that drops `dbKey` now fails loudly instead of silently rerouting a tenant. The local copy this repo used to keep in step with VR_Secure_API is deleted along with the original: one decision, one implementation, and it lives outside every consumer repo now.
- Deps: `@hillbombcreations/schemas` and `@hillbombcreations/tier-quotas`. **Read both pins from `package.json` on the deployed line.** Consumers of these two packages are NOT all on the same major, and that disagreement is the fact worth knowing, not the numbers.
...
- Cold-start: several Lambdas means 4 cold-start warm-up paths, don't share global state across them.
