...
- **On an ISR site that only holds if the build and the runtime agree on `SITE_RENDER_MODE`.** If the site's stored Amplify buildSpec does not grep `SITE_RENDER_MODE` into `.env.production`, cached pages (home first) are built as ISR and never regenerate, so an owner's edit shows on every `no-store` page and never on home until a rebuild. Seen on vivreal.io; the same configuration was found and fixed on Dougs Kitchen and The Comedy Collective 2026-09-16. Diagnosis: `vivreal-templates-knowledge`, "ISR render mode".
