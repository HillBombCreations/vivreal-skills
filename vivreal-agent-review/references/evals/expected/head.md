...
`secret-in-tree` hit on this very file, which only fires when the file is *present*, and no `ref-unresolved` for the citation at `:22`, which resolved. On the archive tree
...
A third output-path candidate was previously listed here, `agents/short-form-editor.md:160`, justified by the word "writes". That word is not
