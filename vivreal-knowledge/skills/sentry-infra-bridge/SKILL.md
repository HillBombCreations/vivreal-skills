...
explains the throw (e.g. a `TypeError` in `updateGroupTier`). That's a `coder` fix, not an ops dig, no CloudWatch metric will "explain" a logic bug. Escalating it to `vivreal-ops` just burns time.
