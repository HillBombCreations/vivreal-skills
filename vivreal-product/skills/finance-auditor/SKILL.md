...
- **AWS is NOT flat, model it as `usage * 1.091 + domain_registrations`.** Measured 2026-09-24 (Cost Explorer `UnblendedCost`, prod payer account): the bill ran $37.46, $30.27, $36.58, $62.13, $167.45, $170.37 for 2026-04 to 2026-09, so the old "$35/mo FLAT" is low by ~4.6x. **Tax is 9.1% of USAGE only** (9.05/9.04/9.10/9.08/9.12/9.09% for six straight months), and **Amazon Registrar is a separate UNTAXED record type** ($71.00 in 2026-08, $87.00 in 2026-09) that is a domain pass-through netting against domain revenue, not infra. Group by `RECORD_TYPE` before quoting any rate: tax over the whole 2026-08 bill reads 4.8%, over usage it reads 9.12%, unchanged. The line that moves is **Amplify build minutes** ($11.32 to $42.19, peak $56.88, 67-81% build rather than hosting), which tracks **DEPLOYS, not customers**. WorkMail is FALLING ($15.39 to $3.09) and is no longer the biggest line; WAF is new since 2026-07 and rising. Lambda is still $0. **Atlas** (billed by MongoDB directly, NOT AWS): $0 free → **~$60 M10** → **~$150 M20** → **~$400+ M30**, and it tracks **PEAK CONCURRENCY, not signups** (each warm Lambda container holds ~3-15 Mongo connections today; cap ÷ per-container footprint, not ÷ `maxPoolSize`, is the concurrency ceiling). Measured 2026-08-31: the 500-cap cluster died at **~54 concurrent Client API containers** (roughly one episode every 5 days, not daily). **Owner decision, 2026-09-15: stay on the shared tier** rather than take M10. The fix is a connection-handling rewrite (`@hillbombcreations/mongo-connection`, eliminates orphaned clients that were the larger cost) plus reserved-concurrency caps sized against the existing 500, not a tier upgrade. Don't model an M10 spend as imminent; it's back-pocket, revisited only if the fleet alarm fires again post-fix.
...
- Scale ladder: 50→$2.25k, 500→$22.5k, 1k→$45k, 5k→$225k/mo. Fixed infra is **~$83/mo all-in as of 2026-09** (AWS usage $76.42 + tax $6.95, Atlas $0 on the shared tier), excluding the Amazon Registrar domain pass-through. The old ~$105-110 was wrong twice: it priced an M10 nobody took, and it added WorkMail on top of an AWS total that already contained WorkMail.
