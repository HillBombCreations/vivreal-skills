...
description: 'Use when reasoning about how a customer SITE gets built and deployed — the Step Functions deploy pipeline (Vivreal_EventHandler), how ALL customer Amplify apps build from the shared Vivreal_Templates `stable` channel branch (Phase 2, 2026-07-15 — per-site branches are DEAD), the promote-stable release workflow (workflow_dispatch; FFs main→stable, rebuilds every site), domain association via Route53 plus the domain purchase/transfer-in sagas, and what to check when a site is stuck pending/failed or "my Templates/renderer change isn''t showing on a live site". Covers the ordered Deploy-Site states (ASL Resource ARNs use vh-site-deployment-{name}; deployed functions are vh-${stage}-{name}; typos preserved), the seedCollections front state, Amplify env-var injection, and the createAmplifyApp accessToken 255-char failure. Triggers on: site deploy, Amplify, Step Functions, vh-site-deployment, createGithubBranch, associateDomain, seedCollections, markSiteLive, promote-stable, release to customer sites, renderer not showing on site, domain transfer, createAmplifyApp ValidationException.'
---
...
State machine = Deploy-Site. **FOUR entry points start it**: `createSiteCollectionData` (mode `hosted_by_us`), `POST /api/deploySite` (blank first deploy), `redeploySite` (VR_Secure_API, since 2026-07-29), and `updateSiteEnvVars`'s rebuild (EventHandler, since 2026-07-29). Both of the latter previously did a direct Amplify StartJob with NO terminal-status writer — every such redeploy/rebuild hung `deploying` forever. Ordered states (each its own Lambda — deployed function names are `vh-${stage}-{name}`, the ASL Resource ARNs use `vh-site-deployment-{name}`; **typos are real, do NOT rename**):
...
- **`markSiteLive` / `markSiteFailed`** — terminal; sets `sites.deployment.status`. Initial status is `deploying` (`createSiteCollectionData.js`); the `POST /api/siteCollectionData` response returns `queued` (`controllers/createSiteCollectionData.js`); terminal is `live` (`markSiteLive/index.js`) or `failed`. No `pending` is ever written. `markSiteLive` also enqueues a push notification to VR_Main_API's `vivreal-notification-queue` on a template-instantiated site's first live deploy (runs after the critical DB write, 5s-raced, failures swallowed — never fails a deploy); its Lambda timeout is 75s. (A legacy `sync_conflict` value may linger on old docs; the template-sync webhook that wrote it was deleted in Phase 2 — nothing produces it anymore.)

### State machine is NOT in IaC
The Deploy-Site state machine pre-dates the serverless setup; VR_Secure_API invokes it by a static ARN (`STATE_MACHINE_ID` — `hb-api-secrets` is RETIRED; secrets Phase 2 moved everything to `vivreal/prod/*` Secrets Manager + `/vivreal/prod/*` SSM, and the ARN also resolves from an SSM param EventHandler reads as `DEPLOY_STATE_MACHINE_ARN`). Source of truth = `Vivreal_EventHandler/docs/ops/deploy-site-state-machine.asl.json`, pushed by `scripts/update-state-machine.sh` (idempotent; CI runs it after `serverless deploy` on `main`). **Console edits are drift and get clobbered.** (`update-state-machine.sh` covers Deploy-Site only — the domain purchase/transfer sagas ship via their own `deploy-domain-purchase-saga.sh` / `deploy-domain-transfer-saga.sh`.)
...
- **The allowlist that matters is each app's STORED buildSpec, not `buildSpec.js`** (2026-09-16). Editing `src/shared/amplify/buildSpec.js` changes no existing app; only `createAmplifyApp` (create, and UpdateApp on each state-machine redeploy), the `updateSiteEnvVars` self-heal and `scripts/backfill-revalidate-webhooks.js --execute` rewrite it. A key present in the Amplify env but not grepped is visible to `next build` and invisible to the running server: `SITE_RENDER_MODE=isr` built vivreal.io's home page as ISR that the runtime could never regenerate, so edits never showed there until a rebuild (Dougs Kitchen and The Comedy Collective had the same gap). Before trusting any env var to reach a site, read `aws amplify get-app --query app.buildSpec`. Fix procedure and fleet state: `vivreal-eventhandler-knowledge`, "Stored buildSpec".
- **Per-site cache-invalidation provisioning** — when `SITE_CACHE_TTL_SECONDS` is set, each deploy mints a per-site `REVALIDATE_WEBHOOK_SECRET` and auto-upserts a `webhooks` subscription so on-publish revalidation works out of the box (`createAmplifyApp/index.js`; gated via `serverless.yml`).
