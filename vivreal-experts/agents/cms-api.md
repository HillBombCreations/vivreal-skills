...
- HandleMedia Lambda: presigned S3 upload URLs. Actual S3 bucket is `{group.type}-{group.key}` (getPresignedUploadUrl.js:19; createIntegrationObject.js:47; updateIntegrationObject.js:32). The `vivreal-` prefix is the CloudFront media URL only (buildMediaUrl.js:12, `MEDIA_BUCKET_PREFIX`) — not the bucket name.
...
- `collectionObj.refID` is a string. `collectionObjectSchema` now indexes `collectionObj.refID`, `publishDate`, `approvalStatus` (collectionObjectSchema.js:96-98); only `groupID` is genuinely missing.
