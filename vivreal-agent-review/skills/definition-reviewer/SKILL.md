...
Measured coverage on the Claude Code version recorded in the ledger header: it validates the manifest, skills, agents
...
2. Verify the fences before extracting: lines 355 and 778 bound `structural_check.py`
   948 and 1326 bound `judge.py`. If they are not fences, the kit was edited, re-locate them rather than extracting a wrong range.
