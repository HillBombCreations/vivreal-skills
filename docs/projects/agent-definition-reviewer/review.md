...
`ref-unresolved` findings are **output** paths … and are the documented limitation in the rule", without pinning their severity. `rules.md:157` says downgrade those "to a NOTE or drop it", which
...
- **No output verb.** `rules.md:198-200` requires the surrounding sentence to say *writes, produces
  emits, generates, regenerates, or creates*. I grepped lines 150 to 172 for all six stems: the only hit
...
### N2 to 6a's `- [` tell is confounded with ordinary markdown, and 6a fails silently
...
**Genuinely fine to leave**, the tell is one of several, and every corpus instance lands correctly, but it is the mechanism by which 6a will drift between runs.
...
The coder is right that `rules.md:150-154` says this path should carry a `<target>/` placeholder, the plugin violates its own corollary. It matters more than it looks, because a marketplace-distributed
...
**(2) `instagram.com/p/` emits a NOTE.** Confirmed as a false positive, and **worse than reported**, under Reading A of clause 6 it is a full **ERROR**, not a NOTE (N3). The coder's diagnosis is right:
...
(6 × 6b instructional in the output templates, which a correct 6a implementation suppresses to 0, plus 2 suffix-fallback disclosures on `marketplace.json`). **0 errors / 0 warnings holds.**
...
Note the agent md5 moved from pass 1's `7a5dc7f5…` to `7c29a20…`, expected, the remediation edited it, and the mirror moved with it, which is the thing that actually matters.
