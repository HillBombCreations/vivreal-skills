...
- Auth flow: signup, Cognito `SignUpCommand`, verify, then login returns JWTs. Login (standard and SSO) mints `ctxPayload.dbKey` by **reading the stored placement**: `resolvePlacement(group)` from `@hillbombcreations/tenant-placement`. There is no tier mapping and no fallback. A group projection that drops `dbKey` makes it THROW rather than silently reroute, which is the entire point of the package. `deriveDbKey()` is deleted. If you find it, or `databaseDict[group.tier]`, or any inline tier-to-database ladder, that is the bug, not the pattern.
