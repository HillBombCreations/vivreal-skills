...
A **Next.js 16 web app** (App Router, React 19, TS strict, Tailwind 4 + Radix) — NOT React Native despite the folder name. Enterprise CMS management portal, v0.2.0. `basePath: '/app'`. For full depth (proxy-route snapshot, all gotchas), read `C:\repos\Vivreal_Portal_Mobile\CLAUDE.md` — this is a lean digest; note CLAUDE.md is itself stale (2026-07-15 — predates the 180-route count, the `/outreach/reach-out` rename, the `isUnlimited` rename, and Basic-tier `canHidePoweredBy()`). July-2026 surfaces: per-site analytics dashboard (`Sites/SiteDetail/AnalyticsPanel`, mounted via `AnalyticsEditor` from the site detail Analytics chip), Instagram comments-moderation + live DM inbox (Human Agent window removed), dedicated IG/FB publish dialogs with inline-play tiles, Studio LeftRail chrome/SEO/Reservation editors, outreach reach-out console (`/outreach/cold-call` renamed to `/outreach/reach-out`; channel SegmentedControl Calls/LinkedIn/Email), flag-gated two-level template-kit picker + `sites/instantiateTemplate` (renderer 1.31.0), managed-domain transfer-in (Transfer tab in `CustomDomainSection`, `TransferDomainTab`/`TransferDomainDialog`, `/sites/[siteId]/domain/connect` page), public demo-account claim flow (`(public)/claim/[token]`), 10-screen `CompanyProfileWizard` onboarding, unified `NotificationsPanel` (replaces `OutreachNotificationToggle`), overage billing (`OverageBillingSection`/`SpendingCapSection`), NewSiteDialog template gallery + Blank-site first-deploy flow.
