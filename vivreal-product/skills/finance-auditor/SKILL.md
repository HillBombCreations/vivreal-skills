...
- **Anthropic** (direct Claude API, not Bedrock): ~$4/customer blended WITH prompt caching (−45%) + a leaner model for routine actions. The tail risk: a maxed **Pro Plus** customer costs ~$50-80/mo optimized (vs ~$250 uncached) against a $119 plan. An "action" = one tool call; one request = N+1 model calls; **no prompt caching ships today** (the standing fix-before-AI-GA item).
...
4. **Check the margin floor.** State the gross margin before and after. Flag anything that pushes a tier below the ~80% floor (or makes a tier margin-negative at max use, like uncached Pro Plus AI).
