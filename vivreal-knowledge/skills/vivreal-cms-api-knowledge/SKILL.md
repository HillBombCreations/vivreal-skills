...
then CMS, then Main (15:30), Client (15:45), and portal (16:00). Incremental
...
`processMediaFields` → `@shared/generateImageDerivatives.js` using **jimp (pure JS, arm64-safe)**: widths 320/640/1280, downscale-only, output **JPEG q80 or PNG (alpha)** at sibling keys `{originalKey}.{width}.{jpg|png}`, persisted as `meta.derivatives` + `meta.derivativeFormat`, consumed by VR_Client_API's signed `srcset`. Best-effort (failure never blocks the upload). A WebP/ffmpeg approach was tried and reverted (x86_64 layer + no jimp webp codec).
...
- **New `GET /tenant/dashboardInsights` aggregate endpoint** (2026-08), stock thresholds, signup dedup, cadence windows (this-week), plus a supporting `integration_objects` cadence index; shipped with several correctness fixes (stock `$lte` not `$lt`, signups tautology removed, object-shaped display-name guard, excluded-variant stock flag, bounded finds). Backs the portal's dash-insights "Your week" sentences.
- **Atlas half-open connection teardown fix**, the connection close is no longer awaited synchronously, which was wedging the Lambda container on a half-open socket.
- ESLint (correctness rules) + a 100%-coverage test suite + husky push gate are now in place, no GitHub Actions test workflow exists, so the hook is the only automated gate before merge.
