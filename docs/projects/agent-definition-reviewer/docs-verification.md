...
- The **nesting eval case is safe to build.** "Subagents can spawn subagents, three layers below the main conversation" is doc-confirmed verbatim. Assert the *behavior* ("subagents CAN nest") rather than the literal integer `3`, the number is a per-session default settable via `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`, and it was `5` from v2.1.172 to 216 and `1` from v2.1.217 to 218. An eval hard-coding `3` is a version-dated assertion and will rot.
