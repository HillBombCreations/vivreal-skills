...
3. **Audit is deferred** until sub-projects 1 to 3 land, then dogfoods the new
...
After 1 to 3 are merged, run `/orchestrate --workflow=audit` against the changed
...
This sub-project gets its own scope/spec once 1 to 3 land; it is explicitly out of
