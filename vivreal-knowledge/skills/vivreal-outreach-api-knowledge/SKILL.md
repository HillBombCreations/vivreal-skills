...
- **`cronTick`** — EventBridge `rate(1 minute)`, 300s timeout; sends due sequence steps. Overlapping ticks possible → send paths must be **idempotent** (rate limiter + scheduling in `src/cron/`). Skips senders with empty `fromAddress`. 0 enrolled = 0 sends.
- **`processBounce`** — SES bounce/complaint via SNS → suppression list.
- **`processInboundReply`** — SES receipt rule → S3 → SNS → parse + forward (decodes the `+r{code}` reply token back to an enrollment). Raw `.eml` in the inbound bucket expires after 90 days (not an archive).
...
- Stored as **collection objects in tenant DBs** under system collection groups (`provision/createSystemGroups.js`): `Outreach Sequences` (audited+versioned), `Outreach Enrollments` (high-volume, audit+versioning skipped), `Outreach Contacts`, `Outreach Senders` (seeded with three founders). Companies are a **first-class entity** linked from contacts via `companyId`.
- **Two-entity model:** Contact = verified email + first/last + `companyId` → Company; Company = name + website + ≥1 hook. Provision is idempotent; legacy `__outreach_*` group names migrate via `renameSystemGroups.js` (LEGACY_NAME fallback bridges deploy→migration).
- **Sequence step:** `{ idx, delayHours (relative; /24 = business days), kind:'email'|'call', subject, body }`. `subject:null` is NOT auto-"Re:". `industryPersonalization` needs `{{customField.<key>}}` tokens (bare names don't resolve).
- **Variable freeze:** sequence variables freeze at **enrollment-snapshot** time (`db/contactSnapshot.js`); company fields **overlay** at enroll/import. The cron renders the snapshot, NOT live contact data. Enroll is manual: `POST /sequences/enroll`.

## SES send + reply routing

- Sends **From `@vivreal.io`** (`OUTREACH_FROM_DOMAIN`; `SESCrudPolicy` is a HARD cutover on that domain).
- Reply-To carries a `+r{code}` routing token (`email/enrollmentCode.js`) on `replies.vivreal.io`, decoded back to an enrollment on inbound. CC's the real founder mailbox; Workspace dual-delivery copies replies to SES.
