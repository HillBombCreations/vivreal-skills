...
- **Deployed portal facts as of 2026-09-08:** v0.18.0, commit `04ac7b62`, Amplify app `d2e6e3kdfrrxak`, branch `stable`, **job 35**. Nav favourites are server-backed and real: `PUT/GET /api/proxy/user/nav-favorites`, one `Vivreal.userpreferences` document keyed by the Cognito login email with **no groupID**, so the choice follows the person and not the group; deleting the `nav_favorites` cookie and the `nav_favorites_seeded` localStorage flag rebuilds both from the account. There is **no landing-page setting anywhere in the product**: `src/app/(public)/launch/page.tsx` hardcodes `router.replace("/dash")`, and the "Where you land when you open the app" row is a description, not a control. The panel is also fully interactive at 1280 and changes nothing about the desktop sidebar. (Walk 10 sections 2 and 3.)
