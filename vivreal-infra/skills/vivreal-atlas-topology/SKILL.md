...
5. **`appName` = `process.env.AWS_LAMBDA_FUNCTION_NAME`**, passed as a client option, never the shared `NAME=hillbomb_api` SSM value that makes the whole fleet indistinguishable in Atlas today.
