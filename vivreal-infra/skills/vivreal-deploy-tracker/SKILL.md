...
named by the group document. **Read `group.dbKey` from `Vivreal.groups`.** Do not compute it
from the tier: that ladder is deleted everywhere, and a tenant database is a placement, not a plan.
```
...
to the `vivreal-ops` agent) wraps steps 1 to 2, dispatch `vivreal-ops` if you have it and want a one-shot.
...
- **Deploy-Site is not the only state machine**, the domain-purchase saga and the domain-transfer-in saga
  ...
  5 to 10 day window) is NOT a stuck site deploy; don't match its executions against a site's deploy.
