...
The CMS backend: collections, collection objects, integrations, media uploads, inbound platform webhooks, audit logging, content versioning. Maps to `NEXT_PUBLIC_CMS_URL`. Express + serverless-express on Lambda (Node 20, arm64), JavaScript, MongoDB/Mongoose, Cognito JWT via API Gateway, SAM. Read `C:\repos\VR_CMS_API\CLAUDE.md` for depth (CLAUDE.md refreshed 2026-07-21 — current as of this sync). One caveat: its Routes list is abbreviated vs the actual routers (approvals, webhooks, DM endpoints absent) — trust the src/ routers for route inventory.
