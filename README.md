...
| **`vivreal-experts`** | 8 read-only system-expert consultants for the Vivreal ecosystem: `portal`, `cms-api`, `secure-api`, `main-api`, `client-stack`, `event-handler`, `outreach-api`, `sites-stack` (renderer + Templates + migrator/site-loader). | ⚠️ Enable when you work across the Vivreal backend/portal repos. |
