...
|---|---|---|
| **`vivreal-principal`** | 7 repo-agnostic principal-level agents: `coordinator`, `coder`, `architect`, `researcher`, `reviewer`, `designer`, `growth-auditor`. The coordinator/coder/architect/researcher/reviewer each merge the former standalone `principal-*` agent with its `vivreal-workflow` counterpart: an approved `plan.md`/`design.md` (or `research.md`/`investigation.md`) is an OPTIONAL input, followed as the spec with zero scope creep when given, and the agent runs standalone with the same principal-caliber judgment when none is given. | ✅ Yes, generalist. Safe and useful in any repo. |
| **`vivreal-workflow`** | The Vivreal bug/feature workflow: slash commands (`/orchestrate`, `/coordinator`, `/research`, `/design`, `/implement`, `/test`, `/reviewer`, `/vuln-fix`, …) that dispatch the shared `coordinator`/`researcher`/`architect`/`coder`/`reviewer` agents from `vivreal-principal` in artifact-driven mode, plus this plugin's own `delivery-orchestrator`/`documenter`/`system-doc-writer`/`tester`/`vuln` role agents, the `shared-standards` skill, and the `verification-discipline` skill (the false-zero mechanisms, the positive control, and why a gate can be enabled and still incapable of failing). Requires `vivreal-principal` installed, the workflow commands dispatch its agents by bare name. | ✅ The repo-agnostic workflow generalists (coordinator/researcher/coder/reviewer, now in `vivreal-principal`, plus this plugin's tester) work anywhere; the docs/bugs flow is most useful in Vivreal repos. |
...
**Agents**, each agent's `description` is written so Claude auto-selects it from your intent; you don't name the agent. "How should we structure this API and then build it?" → `coordinator`; "review this before I ship" → `reviewer`; "trace this feature end-to-end across repos" → `fullstack-tracer`; "what happened when I clicked publish?" → `sentry`; "why is this Lambda throttling / is Atlas saturated?" → `vivreal-ops`.

The four investigation-flavored agents are deliberately non-overlapping so they don't steal each other's dispatches: **`architect`** designs (before code); **`researcher`** investigates from source code (file:line); the **`sentry`** agent reads Sentry telemetry (error timelines); the **`vivreal-ops`** agent reads running AWS/Atlas state (live config, concurrency, executions, connection saturation). Each description names and disclaims the others.

The three **`vivreal-product`** audit agents are likewise disjoint from the existing roles. **`finance-auditor`** audits the COST side (margin, infra/AI cost, scale economics), distinct from the **`growth`/`growth-auditor`/`growth-advisor`** agents, which own the REVENUE side (funnel, conversion, churn, CAC). **`marketing-auditor`** CRITIQUES copy against the brand voice + best-practices, distinct from **`content-planner`/`content-creator`** (which PRODUCE content) and the growth auditor (positioning/funnel strategy). **`ux-critic`** CRITIQUES screen usability read-only, distinct from **`designer`** (which DESIGNS and BUILDS) and from growth (funnel metrics). Each description names and disclaims those it could collide with, and each routes fixes out (to a coder / content / design agent) rather than implementing.
...
- **`vivreal-workflow` needs `vivreal-principal` installed**: its commands (`/coordinator`, `/orchestrate`, `/design`, `/implement`, `/research`, `/investigate`, `/reviewer`, `/bug-review`, `/plan`) dispatch the `coordinator`/`researcher`/`architect`/`coder`/`reviewer` agents by bare name via the Agent tool, and those agents now live in `vivreal-principal`, not in `vivreal-workflow` itself. Without `vivreal-principal` installed, that dispatch resolves to nothing.
