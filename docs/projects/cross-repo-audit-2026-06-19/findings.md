...
2. **VR_CMS_API, coupon quota check is non-atomic (TOCTOU).**
   `src/shared/enforceCouponQuota.js:68-87`. `countDocuments` then a separate `create`/`update`, concurrent requests at the cap (`maxActiveCoupons`) can both pass and both insert. Monetized gate.
